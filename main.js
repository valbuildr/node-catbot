...
const bot = new discord.Client({ intents: [discord.GatewayIntentBits.Guilds, discord.GatewayIntentBits.GuildMessages, discord.GatewayIntentBits.MessageContent, discord.GatewayIntentBits.GuildMembers, discord.GatewayIntentBits.GuildPresences], allowedMentions: {repliedUser: false} });
...
const textCmdsFiles = fs.readdirSync(textCmdsPath).filter(file => file.endsWith('.js'));
...
const slashCmdsFiles = fs.readdirSync(slashCmdsPath).filter(file => file.endsWith('.js'));
